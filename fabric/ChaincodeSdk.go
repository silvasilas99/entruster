...
func RegisterMetadataOnNetwork(
	...
	req metadata.MetadataModel
) error {
    fmt.Printf("--> Submit Transaction: RegisterMetadata | DID: %s\n", req.DID)
...
func GetAllMetadataFromNetwork(
    ...
    var metadataList []metadata.MetadataModel
    err = json.Unmarshal(result, &metadataList)
    if err != nil {
        return nil, fmt.Errorf("chaincodeSdk.GetAllMetadata: Internal error. Failed to unmarshal metadata list: %w", err)
    }
